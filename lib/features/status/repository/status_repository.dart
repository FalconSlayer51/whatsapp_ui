...
class StatusRepository {
  ...
  void uploadStatus({
    ...
    try {
      ...
      // showSnackBar(context: context, content: 'image added to storage');
      // List<Contact> contacts = [];
      // contacts = await FlutterContacts.getContacts(withProperties: true);
      // showSnackBar(context: context, content: contacts.length.toString());

      //    showSnackBar(context: context, content: '$contacts');
      ...
      if (userDataFirebase.docs.isNotEmpty) {
        for (var doc in userDataFirebase.docs) {
          uidWhoCanSee.add(doc.data()['contactId']);
        }
      }
      // for (int i = 0; i < contacts.length; i++) {
      //   //   showSnackBar(context: context, content: contacts[i].phones[0].number);
